#!/usr/bin/env python3
"""
sync_notes.py – Auto-sync Markdown notes to docs/ folder for Just the Docs Jekyll theme

- Converts all .md files under source folders (e.g. reading_notes/) into
  docs-ready Markdown with YAML front matter
- Adds title, parent section, and nav_order
- Auto-generates index.md with section TOC
- Skips unchanged files using content hashing
- Supports --dry-run and --clean modes
...
# Custom mapping for pretty titles in the sidebar
section_title_map = {
...
def read_file_clean(path):
    """Reads a UTF-8 Markdown file and strips any BOM characters."""
...
def parse_chapter_title(filename):
    """Parses 'chapter-01-title.md' into (1, 'Chapter 1 – Title')"""
...
def load_hashes():
    """Loads the sync cache from .sync_hashes.json if present."""
    if HASH_FILE.exists():
...
def create_or_update_section_index(
    ...
    """Creates or updates index.md in a section with a TOC."""
...
def sync_notes(dry_run=False, clean=False):
    """Main function to sync all notes into the docs/ folder."""
    ...
    print(
        "🎉 Dry run complete."
        if dry_run
        else ("🎉 Sync complete." if any_synced else "🔁 All up to date.")
    )
